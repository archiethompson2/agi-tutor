...
def build_system_prompt(context: dict) -> str:
    ...
    if tinfo:
        lines.append(
            f"Schedule -> sessions/week: {tinfo.get('sessions_per_week','?')}, " +
            f"hours/session: {tinfo.get('hours_per_session','?')}, " +
            f"school year end: {tinfo.get('school_year_end','?')}."
        )
    ...
    # Base tutoring turn format (kept for context)
    base_block = (
    ...
    # OVERRIDE block: stricter, item-level schema (no triple-quotes; plain string with \n)
    override_block = (
